...
const useHandleRegister = (role) => {
    const [formData, setFormData] = useState({ u_login: '', u_email: '', u_pass: '', guardian_token: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/wp-json/family-connect/v1/register', {
                method: 'POST',
            ...
            if (response.ok && data.success) {
                setStatus('success');
                setFormData({ u_login: '', u_email: '', u_pass: '', guardian_token: '' });
                if (data.token) {
                ...
                const errorMap = {
                    ...
                    'user_exists': 'Użytkownik o takim loginie lub e-mailu już istnieje.'
                };
                setMessage(errorMap[data.error] || 'Wystąpił nieoczekiwany błąd.');
            }
        } catch (err) {
            setStatus('error');
